...
(defn- make-ctx
  ([locals]
   (make-ctx locals {}))
  ([locals {:keys [available-modules imported-decls interfaces]}]
   {:locals locals
    :mutable #{}
...
(defn- imported-runtime-available?
  ...
  (let [type-name (patterns/declared-type-name type-expr)]
    (if-let [module (or (get-in ctx [:imported-decls type-name :module])
                        (some (fn [[module interface]]
                                (when (some #(= type-name (:name %))
                                            (:decls interface))
                                  module))
                              (:interfaces ctx)))]
...
(defn- infer-seq-empty
  [expr ctx]
  (result (list 'Seq (:element-type expr)) ctx))

(defn- infer-seq-append
  [expr ctx decls]
  (ensure-arity= 2
                 (count (:args expr))
                 {:expr expr})
  (let [seq-type (infer-expr-type (first (:args expr)) ctx decls)
        element-type (ensure-seq-type seq-type
                                      {:expr expr
                                       :arg (first (:args expr))})]
    (ensure-type= element-type
                  (infer-expr-type (second (:args expr)) ctx decls)
                  {:expr expr
                   :arg (second (:args expr))})
    (result seq-type ctx)))
...
(def ^:private expr-inferers
  ...
  :record-get infer-record-get
   ...
   :env-get (fn [expr ctx decls] (infer-primitive-unary expr ctx decls 'String '(Option String)))
   :process-run (fn [expr ctx decls]
                  (ensure-arity= 2
                                 (count (:args expr))
                                 {:expr expr})
                  (ensure-type= '(Seq String)
                                (infer-expr-type (first (:args expr)) ctx decls)
                                {:expr expr
                                 :arg (first (:args expr))})
                  (ensure-type= 'Bytes
                                ...
                                {:expr expr
                                 :arg (second (:args expr))})
                  (result 'ProcessResult ctx))
   :http-listen (fn [expr ctx decls] (infer-primitive-unary expr ctx decls 'Int 'HttpServer))
   :http-port (fn [expr ctx decls] (infer-primitive-unary expr ctx decls 'HttpServer 'Int))
   :http-accept (fn [expr ctx decls] (infer-primitive-unary expr ctx decls 'HttpServer 'HttpRequest))
   :http-respond (fn [expr ctx decls]
                   (ensure-arity= 3
                                  (count (:args expr))
                                  {:expr expr})
                   (ensure-type= 'HttpServer
                                 (infer-expr-type (first (:args expr)) ctx decls)
                                 {:expr expr
                                  :arg (first (:args expr))})
                   (ensure-type= 'HttpRequest
                                 (infer-expr-type (second (:args expr)) ctx decls)
                                 {:expr expr
                                  :arg (second (:args expr))})
                   (ensure-type= 'HttpResponse
                                 (infer-expr-type (nth (:args expr) 2) ctx decls)
                                 {:expr expr
                                  :arg (nth (:args expr) 2)})
                   (result 'Unit ctx))
   :http-close (fn [expr ctx decls] (infer-primitive-unary expr ctx decls 'HttpServer 'Unit))
   ...
   :seq-empty (fn [expr ctx _decls] (infer-seq-empty expr ctx))
   ...
   :seq-append (fn [expr ctx decls] (infer-seq-append expr ctx decls))
...
(defn- check-fn-decl
  ...
  (let [base-ctx (make-ctx (module-fn-env module decls)
        ...
        ctx (reduce (fn [acc param]
                    ...
                    base-ctx
                    (:params decl))
        ensure-ctx (bind-local ctx '__airj_result (:return-type decl))
        body-type (infer-expr-type (:body decl) ctx decls)]
    (doseq [contract (:requires decl)]
      (check-bool-contract contract
                           ctx
                           decls
                           {:fn (:name decl)
                            :contract contract}))
    (doseq [contract (:ensures decl)]
      (check-bool-contract contract
                           ensure-ctx
                           decls
                           {:fn (:name decl)
...
;; {:version 1, :tested-at "2026-03-16T07:25:12.687474-05:00", :module-hash "803754947", :forms [{:id "form/0/ns", :kind "ns", :line 1, :end-line 4, :hash "1214713234"} {:id "form/1/declare", :kind "declare", :line 6, :end-line 6, :hash "-750848269"} {:id "form/2/declare", :kind "declare", :line 7, :end-line 7, :hash "83426057"} {:id "defn-/fail!", :kind "defn-", :line 9, :end-line 11, :hash "765867407"} {:id "defn-/decl-map", :kind "defn-", :line 13, :end-line 28, :hash "-2115152470"} {:id "defn-/fn-type", :kind "defn-", :line 30, :end-line 35, :hash "1095575499"} {:id "defn-/bottom-type", :kind "defn-", :line 37, :end-line 40, :hash "475588588"} {:id "defn-/bottom-type?", :kind "defn-", :line 42, :end-line 44, :hash "1715044112"} {:id "defn-/result", :kind "defn-", :line 46, :end-line 49, :hash "-1781274230"} {:id "defn-/local-type", :kind "defn-", :line 51, :end-line 53, :hash "-1632824710"} {:id "defn-/mutable-local?", :kind "defn-", :line 55, :end-line 57, :hash "1438220795"} {:id "defn-/bind-local", :kind "defn-", :line 59, :end-line 61, :hash "1658319653"} {:id "defn-/bind-mutable", :kind "defn-", :line 63, :end-line 67, :hash "-1396184064"} {:id "defn-/with-loop-types", :kind "defn-", :line 69, :end-line 71, :hash "208229580"} {:id "defn-/without-loop-types", :kind "defn-", :line 73, :end-line 75, :hash "932574932"} {:id "defn-/fn-decl-type", :kind "defn-", :line 77, :end-line 81, :hash "1335497367"} {:id "defn-/module-fn-env", :kind "defn-", :line 83, :end-line 94, :hash "-1474536557"} {:id "defn-/make-ctx", :kind "defn-", :line 96, :end-line 104, :hash "1757516252"} {:id "defn-/literal-type", :kind "defn-", :line 106, :end-line 117, :hash "-2129463170"} {:id "defn-/compatible-types?", :kind "defn-", :line 119, :end-line 123, :hash "1551296287"} {:id "defn-/ensure-type=", :kind "defn-", :line 125, :end-line 129, :hash "1746399400"} {:id "defn-/ensure-arity=", :kind "defn-", :line 131, :end-line 135, :hash "266695403"} {:id "defn-/join-types", :kind "defn-", :line 137, :end-line 143, :hash "-1353034608"} {:id "defn-/enum-pattern?", :kind "defn-", :line 145, :end-line 147, :hash "-31853100"} {:id "defn-/bind-literal-pattern", :kind "defn-", :line 149, :end-line 155, :hash "-2136209369"} {:id "defn-/bind-binder-pattern", :kind "defn-", :line 157, :end-line 161, :hash "1161467246"} {:id "defn-/bind-pattern", :kind "defn-", :line 163, :end-line 171, :hash "-1814155925"} {:id "defn-/infer-local", :kind "defn-", :line 173, :end-line 179, :hash "195910838"} {:id "defn-/infer-record-get", :kind "defn-", :line 181, :end-line 194, :hash "-361239966"} {:id "defn-/imported-runtime-available?", :kind "defn-", :line 196, :end-line 206, :hash "-1572117793"} {:id "defn-/ensure-constructible-type!", :kind "defn-", :line 208, :end-line 212, :hash "908869040"} {:id "defn-/infer-construct", :kind "defn-", :line 214, :end-line 229, :hash "-1360397945"} {:id "defn-/infer-variant", :kind "defn-", :line 231, :end-line 256, :hash "-1236364359"} {:id "defn-/infer-if", :kind "defn-", :line 258, :end-line 266, :hash "-2003732048"} {:id "defn-/infer-match", :kind "defn-", :line 268, :end-line 282, :hash "262906490"} {:id "defn-/infer-call", :kind "defn-", :line 284, :end-line 298, :hash "-1175533470"} {:id "defn-/infer-lambda", :kind "defn-", :line 300, :end-line 313, :hash "-582701779"} {:id "defn-/infer-binding", :kind "defn-", :line 315, :end-line 319, :hash "737212167"} {:id "defn-/infer-let", :kind "defn-", :line 321, :end-line 328, :hash "-310092863"} {:id "defn-/infer-seq", :kind "defn-", :line 330, :end-line 335, :hash "-2028016492"} {:id "defn-/infer-var", :kind "defn-", :line 337, :end-line 343, :hash "325909027"} {:id "defn-/infer-set", :kind "defn-", :line 345, :end-line 353, :hash "1875378186"} {:id "defn-/infer-loop", :kind "defn-", :line 355, :end-line 370, :hash "867251552"} {:id "defn-/infer-recur", :kind "defn-", :line 372, :end-line 386, :hash "-1380623872"} {:id "defn-/infer-try", :kind "defn-", :line 388, :end-line 403, :hash "-481723416"} {:id "defn-/infer-raise", :kind "defn-", :line 405, :end-line 408, :hash "708997410"} {:id "defn-/infer-primitive-unary", :kind "defn-", :line 410, :end-line 416, :hash "300239028"} {:id "defn-/infer-primitive-binary", :kind "defn-", :line 418, :end-line 428, :hash "-679783036"} {:id "defn-/seq-element-type", :kind "defn-", :line 430, :end-line 437, :hash "-948667098"} {:id "defn-/map-value-type", :kind "defn-", :line 439, :end-line 445, :hash "1293096732"} {:id "defn-/ensure-seq-type", :kind "defn-", :line 447, :end-line 451, :hash "1156209343"} {:id "defn-/ensure-map-type", :kind "defn-", :line 453, :end-line 457, :hash "1186104024"} {:id "defn-/infer-seq-unary", :kind "defn-", :line 459, :end-line 464, :hash "-2137238691"} {:id "defn-/infer-seq-element", :kind "defn-", :line 466, :end-line 472, :hash "-1943390793"} {:id "defn-/infer-seq-rest", :kind "defn-", :line 474, :end-line 481, :hash "-1159837909"} {:id "defn-/infer-seq-concat", :kind "defn-", :line 483, :end-line 497, :hash "1566324222"} {:id "defn-/infer-seq-get", :kind "defn-", :line 499, :end-line 512, :hash "662784337"} {:id "defn-/infer-seq-empty", :kind "defn-", :line 514, :end-line 516, :hash "-545967920"} {:id "defn-/infer-seq-append", :kind "defn-", :line 518, :end-line 531, :hash "-780902292"} {:id "defn-/infer-map-empty", :kind "defn-", :line 533, :end-line 535, :hash "-313109806"} {:id "defn-/infer-map-set", :kind "defn-", :line 537, :end-line 554, :hash "-627309182"} {:id "defn-/infer-map-get", :kind "defn-", :line 556, :end-line 569, :hash "1449919953"} {:id "defn-/infer-map-contains", :kind "defn-", :line 571, :end-line 583, :hash "831295168"} {:id "defn-/infer-map-keys", :kind "defn-", :line 585, :end-line 590, :hash "48903908"} {:id "defn-/infer-java-new", :kind "defn-", :line 592, :end-line 596, :hash "-676160678"} {:id "defn-/infer-java-call", :kind "defn-", :line 598, :end-line 612, :hash "1132722729"} {:id "defn-/infer-java-static-call", :kind "defn-", :line 614, :end-line 627, :hash "-1746811078"} {:id "defn-/infer-java-get-field", :kind "defn-", :line 629, :end-line 632, :hash "-146759388"} {:id "defn-/infer-java-static-get-field", :kind "defn-", :line 634, :end-line 636, :hash "595849367"} {:id "defn-/ensure-assignable-field-type!", :kind "defn-", :line 638, :end-line 644, :hash "-97045908"} {:id "defn-/infer-java-set-field", :kind "defn-", :line 646, :end-line 651, :hash "277510053"} {:id "defn-/infer-java-static-set-field", :kind "defn-", :line 653, :end-line 657, :hash "-808600186"} {:id "def/expr-inferers", :kind "def", :line 659, :end-line 817, :hash "-1312304219"} {:id "defn/infer-expr", :kind "defn", :line 819, :end-line 826, :hash "-1459178635"} {:id "defn/infer-expr-type", :kind "defn", :line 828, :end-line 830, :hash "1740972133"} {:id "defn-/check-bool-contract", :kind "defn-", :line 832, :end-line 836, :hash "1265459629"} {:id "defn-/passthrough-invariant-ctx", :kind "defn-", :line 838, :end-line 840, :hash "-1920191979"} {:id "defn-/invariant-ctx", :kind "defn-", :line 842, :end-line 853, :hash "1279370076"} {:id "defn-/check-invariants", :kind "defn-", :line 855, :end-line 862, :hash "-1390776170"} {:id "defn-/check-fn-decl", :kind "defn-", :line 864, :end-line 890, :hash "-762644918"} {:id "defn/check-module", :kind "defn", :line 892, :end-line 906, :hash "86717504"}]}
