...
(def ^:private infer-type-handlers
  {:local (fn [expr ctx] (local-type ctx (:name expr)))
   ...
   :json-parse (fn [_ _] 'Interchange)
   :json-write (fn [_ _] 'String)
   :env-get (fn [_ _] '(Option String))
   :process-run (fn [_ _] 'ProcessResult)
   :http-listen (fn [_ _] 'HttpServer)
   :http-port (fn [_ _] 'Int)
   :http-accept (fn [_ _] 'HttpRequest)
   :http-respond (fn [_ _] 'Unit)
   :http-close (fn [_ _] 'Unit)
   :string->int (fn [_ _] 'Int)
   :string-length (fn [_ _] 'Int)
   :string-trim (fn [_ _] 'String)
   :string-empty? (fn [_ _] 'Bool)
   :seq-empty (fn [expr _] (list 'Seq (:element-type expr)))
   ...
   :seq-append (fn [expr ctx] (infer-type (first (:args expr)) ctx))
